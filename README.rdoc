...
Rather than full or partial text matching, ActsAsMatchable is designed to
...
ActsAsMatchable is used to extend ActiveRecord objects however it does not
utilize any SQL thus making it database agnostic.

Matched attributes (if any) are returned from the pattern_match method call as
